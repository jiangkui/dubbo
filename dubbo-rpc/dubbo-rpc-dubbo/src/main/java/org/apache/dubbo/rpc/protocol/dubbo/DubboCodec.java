...
public class DubboCodec extends ExchangeCodec {
    ...
    /**
     * 对 Consumer 传过来的 Request 进行解码
     * 对 Provider 返回的 Response 进行解码
     */
    @Override
                ...
                if (status == Response.OK) {
                    ...
                    if (res.isEvent()) {
                        ...
                        if (channel.getUrl().getParameter(DECODE_IN_IO_THREAD_KEY, DEFAULT_DECODE_IN_IO_THREAD)) {
                            ...
                            // 在IO线程上解码，则直接进行后续的解码工作
                            result.decode();
                        } else {
                            // 在线程池解码，则直接创建对象，后续会丢到线程池内解码，不占用 IO线程
            ...
            try {
                ...
                if (req.isEvent()) {
                    ...
                    /*
                        ...
                        - IO线程：就是当前线程，Dubbo 把底层通信框架中接收请求的线程称为 IO 线程
                        - 线程派发：如果处理比较耗时，则需要用线程池来处理，原因是：IO 线程主要用于接收请求，如果 IO 线程被占满，将导致它不能接收新的请求。

                        后面会搞一个线程池，如果不在IO线程解码，则会封装成一个 Runnable，调用一次 inv.decode();
                     */
                    if (channel.getUrl().getParameter(DECODE_IN_IO_THREAD_KEY, DEFAULT_DECODE_IN_IO_THREAD)) {
                        inv = new DecodeableRpcInvocation(channel, req, is, proto);
                        // 在 IO 线程上解码，则直接调用，进行解码（调用方法名、attachment、以及调用参数解析出来）
                        // 这里面有些内容
                        inv.decode();
                    } else {
                        // 在线程池解码，则仅创建 DecodeableRpcInvocation 对象，后续会丢到线程池解码，不占用 IO线程
